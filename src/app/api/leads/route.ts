 ...
 */
export const POST = withApiHandler({endpoint: "/api/leads", method: "POST", requireAuth: false}, async (request) => {
	...
	emailService.sendLeadNotification(leadInstance.toEmailData()).catch((err) => {
 ...
 */
export const GET = withApiHandler({endpoint: "/api/leads", method: "GET", requireRole: "admin"}, async (_request, {session}) => {
	// Session is guaranteed by requireRole: "admin"
	const userId = session!.user.id;

	// Get all leads for this realtor, ordered by most recent
	const allLeads: LeadRow[] = await db.select().from(lead).where(eq(lead.realtorId, userId)).orderBy(desc(lead.createdAt));
	...
	const countResult: {count: number}[] = await db.select({count: sql<number>`count(*)`}).from(lead);
