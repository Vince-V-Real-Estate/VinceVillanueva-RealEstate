...
import {Bath, Bed, Building, Calendar, CheckCircle2, ChevronLeft, MapPin, Square} from "lucide-react";
...
/**
 * Renders the presale detail page using database-backed listing data.
 * @async
 * @param params - Route parameters containing the presale listing id.
 * @returns The presale details UI for the requested listing.
 */
export default async function PresaleDetailPage({params}: PresaleDetailPageProps) {
	const {presaleId} = await params;
	const listing = await getPresaleListingById(presaleId);
					...
					<div className="space-y-12 lg:col-span-2">
						<section className="grid grid-cols-2 gap-4 sm:grid-cols-4">
							...
							<div className="flex flex-col items-center justify-center rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-transform hover:-translate-y-1">
								...
								<p className="text-2xl font-bold text-gray-900">{listing.squareFeet.toLocaleString()}</p>
