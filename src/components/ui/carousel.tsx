...
const Slide = ({slide, index, current, showDetailsOverlay, handleSlideClick}: SlideProps) => {
	...
	const {src, title, description, price, address, specs} = slide;
		...
		<div className="perspective-distant transform-3d">
			...
			>
				...
				>
					...
					<div className={`absolute inset-0 flex flex-col justify-end bg-black/40 p-8 backdrop-blur-[2px] transition-opacity duration-300 sm:p-10 ${showDetailsOverlay ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
						<div className="translate-y-4 transform text-left transition-transform duration-300 group-hover:translate-y-0">
							<div className="xs:gap-20 flex w-full flex-row justify-between">
								<span className="mb-2 inline-block rounded bg-white/20 px-2 py-1 text-xs font-bold tracking-wider text-white uppercase backdrop-blur-md">Featured</span>
								<Button className={"block"}>View Details</Button>
							</div>
							{description ? <p className="mb-2 line-clamp-3 text-sm text-gray-100">{description}</p> : null}
				...
				>
					<h2 className="text-xl font-bold drop-shadow-md md:text-2xl lg:text-3xl">{title}</h2>
				</article>
			</li>
		</div>
	);
...
export default function Carousel({slides}: CarouselProps) {
	...
	const [mobileDetailsIndex, setMobileDetailsIndex] = useState<number | null>(null);

	const normalizedCurrent = slides.length > 0 ? ((current % slides.length) + slides.length) % slides.length : 0;
				...
				>
					{slides.map((slide, index) => (
						<Slide
							...
							showDetailsOverlay={isTouchInput && normalizedCurrent === index && mobileDetailsIndex === index}
