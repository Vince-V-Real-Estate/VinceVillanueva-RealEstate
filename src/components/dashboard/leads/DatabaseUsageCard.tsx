...
export function DatabaseUsageCard({usage}: DatabaseUsageCardProps) {
	...
	return (
		<div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
			<h3 className="mb-4 text-lg font-semibold text-gray-900">Database Usage</h3>
			<div className="space-y-3">
				<div className="flex justify-between text-sm">
					<span className="text-gray-600">Leads stored</span>
					<span className="font-medium text-gray-900">
						{usage.currentRows.toLocaleString()} / {usage.estimatedMaxRows.toLocaleString()}
					</span>
				</div>
				...
				<p className="text-xs text-gray-500">{usage.usagePercent.toFixed(1)}% of estimated capacity used</p>
				{usage.usagePercent >= 60 && <p className="mt-2 text-xs text-amber-600">Consider exporting and clearing old leads to free up space.</p>}
			</div>
		</div>
	);
}
