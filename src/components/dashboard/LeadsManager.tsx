...
import {ChevronDown, ChevronUp, LayoutList, StretchHorizontal} from "lucide-react";
...
export function LeadsManager() {
	...
	const [expandedSources, setExpandedSources] = useState<Record<LeadSource, boolean>>({
		listings: true,
		valuation: true,
		call: true,
		newsletter: true,
		...
		<div className="space-y-8">
			...
			<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
				...
				<div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
					...
					<p className="mt-2 text-3xl font-bold text-gray-900">{leadsData?.totalCount ?? 0}</p>
				</div>
				...
				{sources.map((source) => (
					...
					>
						<div className="flex items-center gap-2">
							<div className={`h-3 w-3 rounded-full ${SOURCE_COLORS[source]}`} />
							<p className="text-sm font-medium text-gray-600">{SOURCE_LABELS[source]}</p>
						</div>
						<p className="mt-2 text-3xl font-bold text-gray-900">{leadsData?.bySource[source]?.length ?? 0}</p>
					</div>
				))}
			</div>
			...
			<div className="flex justify-end">
				<div className="inline-flex rounded-lg border border-gray-200 bg-white p-1">
					<button
						...
						className={cn("flex items-center gap-2 rounded-md px-3 py-1.5 text-sm font-medium transition-colors", !isUnified ? "bg-gray-100 text-gray-900" : "text-gray-500 hover:text-gray-900")}
					>
				...
				<div className="space-y-8">
					{sources.map((source) => (
						...
						>
							...
							>
								<div className={`h-4 w-4 rounded-full ${SOURCE_COLORS[source]}`} />
								<h2 className="text-lg font-semibold text-gray-900">{SOURCE_LABELS[source]}</h2>
								<span className="rounded-full bg-gray-200 px-3 py-1 text-sm font-medium text-gray-700">{leadsData?.bySource[source]?.length ?? 0} leads</span>
								<div className="ml-auto">{expandedSources[source] ? <ChevronUp className="h-5 w-5 text-gray-500" /> : <ChevronDown className="h-5 w-5 text-gray-500" />}</div>
							</button>
